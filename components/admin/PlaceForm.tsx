...
export default function PlaceForm({ mode, initialData, lang }: PlaceFormProps) {
    ...
    const [form, setForm] = useState<PlaceFormData>(() => {
        ...
        return {
            ...
            slug: { en: initialData.slug?.en || '', he: initialData.slug?.he || '', ar: initialData.slug?.ar || '' },
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div>
                                <label className="flex flex-col text-sm font-semibold text-slate-700 mb-1">
                                    SEO URL Slug ({activeTab.toUpperCase()})
                                    {activeTab === 'en' && <span className="text-xs text-slate-400 font-normal">Auto-generated by backend</span>}
                                </label>
                                <input
                                    type="text"
                                    value={form.slug[activeTab]}
                                    onChange={e => activeTab !== 'en' && updateLocalized('slug', activeTab, e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
                                    disabled={activeTab === 'en'}
                                    className={`w-full px-4 py-2.5 rounded-lg border border-slate-200 outline-none ${activeTab === 'en' ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'focus:ring-2 focus:ring-blue-500'}`}
                                    placeholder={activeTab === 'en' ? "Auto-generated from English title" : "e.g. jabal-al-shaykh"}
                                    dir="ltr"
                                />
                            </div>
